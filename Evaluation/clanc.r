...
"cvClanc" <-
function(data, id, prior = "equal", active = 1:10, gui = F, prntFrm = NULL) {
...
"buildClanc" <-
function(data, id, cNames, train, active) {
  m = nrow(data)
  n = ncol(data)
  p = length(unique(id))
  nn = as.numeric(table(id))
...
"testClanc" <-
function(data, id, geneNames, fit) {
  m = nrow(data)
  n = ncol(data)
  p = ncol(fit$cntrds)
  cntrds = fit$cntrds
...
"trainClanc" <-
function(data, id, geneNames, prior = "equal") {
  ## data: expression data.  (m x n) matrix of class numeric.
  ## id: class id's.  n vector of class numeric.
