...
if __name__=='__main__':
    ...
    selected_features=np.genfromtxt('F:/brcadata/selected_features.csv', delimiter =',',skip_header =1)

    inputge=np.genfromtxt('F:/brcadata/RNASEQ.csv',dtype  = np.unicode_, delimiter =',',skip_header =1)
    inputcnv=np.genfromtxt('./CNV.csv',dtype  = np.unicode_, delimiter =',',skip_header =1)
    inputmiRNA=np.genfromtxt('./miRNASEQ.csv',dtype  = np.unicode_, delimiter =',',skip_header =1)    

#    namege=np.genfromtxt('F:/brcadata/RNASEQ.csv',dtype  = np.unicode_, delimiter =',')[0,0:inputge.shape[1]]
#    nameme=np.genfromtxt('E:/brcadata/CNV.csv',dtype  = np.unicode_, delimiter =',')[0,1:inputcnv.shape[1]]
#    namemi=np.genfromtxt('F:/brcadata/miRNASEQ.csv',dtype  = np.unicode_, delimiter =',')[0,1:inputmiRNA.shape[1]]
  ...
  #  fname = 'E:/newflder/autre pc/trials_HPMV_maxepochs1000_max_evals50_tanh.pkl'
    fname = 'F:/brcadata/pkl_files/BRCA_trials_evals50_tanh50.pkl'
  
#    fname = 'H:/Nouveau dossier (3)/autre pc/coadpins/coadp_trials_HPMV_maxepochs1000_max_evals50_tanh4.pkl'
    namege= np.genfromtxt('./namege.csv',dtype  = np.unicode_, delimiter =',')
    nameme= np.genfromtxt('./nameme.csv', dtype  = np.unicode_, delimiter =',')
    namemi= np.genfromtxt('./namemi.csv', dtype  = np.unicode_, delimiter =',')
    input= open(fname, 'rb')
#
#        print(pathnames[iii])
  ...
  #      trials =pickle.load(input)        
   #     trials =pickle.load(input)
    ...
    for iterator in range(50) :
        pathname = pathnames[iterator+51][0:len(pathnames[iterator+51])-1].replace("\\","")
        ...
        if mrna_nbr >2 or cnv_nbr >2 or miRNA_nbr>2:
          ...
          if  mrna_nbr >1:
              if cnv_nbr  <2 and miRNA_nbr  <2:
                  ...
                  fig.savefig('F:/brcadata/GE/'+pathname + 'mrna'+'.png',bbox_inches = 'tight')
                  ...
                  if cnv_nbr >1:
                      ...
                      fig.savefig('F:/brcadata/GE/'+ pathname + 'cnv'+'.png',bbox_inches = 'tight')
                      ...
                      if miRNA_nbr>1:
                          ...
                          fig.savefig('F:/brcadata/GE/'+ pathname + 'mirna'+'.png',bbox_inches = 'tight')   
          ...
          fname = './SHAPValuesbrca50.pkl'
          pickle.dump(shapfeat,open( fname, "wb" ))       
          with open('./MVbrca50.csv', 'w') as csvfile:
